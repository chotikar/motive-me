...
void main() {
  ...
  testWidgets('E2E E2E Integration Test: Login, Add Skill, Verify, Edit Profile, Logout', (tester) async {
    ...
    // Tapping the title "Motive Me" forces native blur/unfocus on all inputs,
    // which completely dismisses soft keyboards and browser autocomplete/autofill overlays.
    await tester.tap(find.text('Motive Me'));
    await tester.pump();
    ...
    // Tapping the "Add Skill" header forces native blur/unfocus on all inputs,
    // dismissing soft keyboards and browser autocomplete overlays.
    await tester.tap(find.text('Add Skill').first);
    await tester.pump();
    FocusManager.instance.primaryFocus?.unfocus();
    await tester.pump();
    await Future.delayed(const Duration(seconds: 1));
    ...
    // Pump transition frames to let the profile page route complete
    print('DIAG: Pumping profile page transition frames...');
    await tester.pump();
    await tester.pump(const Duration(seconds: 3));
    ...
    // Pump transition frames to let the edit profile page route complete
    print('DIAG: Pumping edit profile page transition frames...');
    await tester.pump();
    await tester.pump(const Duration(seconds: 3));
    ...
    // Tapping the "Name" label forces native blur/unfocus on all inputs,
    // dismissing soft keyboards and browser autocomplete overlays.
    ...
    // Pump transition frames to let the logout dialog route complete
    print('DIAG: Pumping logout dialog transition frames...');
    await tester.pump();
    await tester.pump(const Duration(seconds: 3));
